...
export default function FavoritesBtn() {
  ...
  const isValidLocation =
    weatherData?.latitude && weatherData?.longitude && weatherData?.place;

  const isAlreadyFavorite = favorites.some(
    (fav) =>
      fav.lat === weatherData?.latitude && fav.lon === weatherData?.longitude,
  );
  ...
  }, [addFavorite, weatherData, isAlreadyFavorite, isValidLocation]);

  const title = isAlreadyFavorite
  ...
  return (
    ...
    >
      <IconFavorite
        className={cnr(
          isAlreadyFavorite && styles.itsFav,
          isAnimating && styles.addingFav,
        )}
      />
    </button>
  );
