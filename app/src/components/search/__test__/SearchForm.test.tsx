...
import type { DropdownSearchProps } from "../../dropdowns/search/DropdownSearch";
...
vi.mock("../../../hooks/useLiveSearch/useLiveSearch", () => ({
  useLiveSearch: vi.fn((search: string) => ({
    data: search
      ? [{ place: search, coords: { latitude: 0, longitude: 0 } }]
      : [],
    isFetching: false,
    flush: vi.fn(),
  })),
...
vi.mock("../../../context/toast/ToastContext", async (importOriginal) => {
  const actual =
    await importOriginal<
      typeof import("../../../context/toast/ToastContext")
    >();
  return {
    ...actual,
...
vi.mock("../../../components/dropdowns/search/DropdownSearch", () => ({
  default: (props: DropdownSearchProps) => {
    if (!props.dropdown) return null;
    return (
      <div data-testid="dropdown">
        <button
          onClick={() =>
            props.onSelect("London", { latitude: 51.5, longitude: -0.1 })
          }
        >
          London
        </button>
...
vi.mock(
  "../../../components/common/speechrecognition/SpeechRecognitionMic",
  () => ({
...
describe("SearchForm", () => {
  ...
  it("handles voice input through mic button", () => {
    render(<SearchForm />);
