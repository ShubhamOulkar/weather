 ...
 */
export async function fetchCityData(city: string, limit: number = 5): Promise<CityData[]> {
    ...
    try {
        ...
        if (!res.ok) {
            console.log(`HTTP Error: Failed to fetch coordinates for ${city}. Status: ${res.status} ${res.statusText}`)
            throw new Error(`HTTP Error: Failed to fetch coordinates for ${city}. Status: ${res.status} ${res.statusText}`);
        }
        ...
        throw new Error(`An unexpected error occurred during geocoding for ${city}.`);
    }
}