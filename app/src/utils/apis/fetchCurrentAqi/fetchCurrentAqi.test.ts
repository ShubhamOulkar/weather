...
describe("fetchCurrentAqi", () => {
  ...
  it("throws an error if env variable is missing", async () => {
    ...
    await expect(fetchCurrentAqi(mockCoords))
      .rejects
      .toThrow("Missing VITE_OPEN_METEO_AQI API endpoint environment variable.");
  });
  ...
  it("throws an error if API returns empty array", async () => {
    ...
    await expect(fetchCurrentAqi(mockCoords))
      .rejects
      .toThrow("OpenMeteo API returned an empty AQI response array.");
  });
  ...
  it("throws an error if AQI data is missing", async () => {
    ...
    await expect(fetchCurrentAqi(mockCoords))
      .rejects
      .toThrow("OpenMeteo response structure is missing current AQI data.");
  });
  ...
  it("throws a wrapped error for non-Error objects", async () => {
    ...
    await expect(fetchCurrentAqi(mockCoords))
      .rejects
      .toThrow("An unexpected error occurred while retrieving AQI data.");
  });
});
