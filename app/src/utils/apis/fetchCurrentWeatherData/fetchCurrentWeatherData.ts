...
export async function fetchCurrentWeatherData({
  latitude,
  longitude,
}: Cooradinates): Promise<CurrentWeatherMetrics> {
  const url: string = import.meta.env.VITE_OPEN_METEO_WEATHER;

  if (!url) {
    throw new Error(
      "Missing VITE_OPEN_METEO_WEATHER API endpoint environment variable.",
    );
  }
  ...
  try {
    ...
    if (!current) {
      throw new Error(
        "OpenMeteo response structure is missing current weather data.",
      );
    }
    ...
    params.current.forEach((variableName) => {
      ...
      if (variableName === "weather_code")
        return (weather_code = variable.value());
    ...
    if (!mainTemperature || weather_code === null) {
      throw new Error(
        "temperature_2m/weather_code is not found in the response.",
      );
    }
