...
export async function fetchDailyWeather({ latitude, longitude }: Cooradinates) {
  ...
  if (!url) {
    throw new Error(
      "Missing VITE_OPEN_METEO_DAILY API endpoint environment variable.",
    );
  }
  ...
  try {
    ...
    if (!responses || responses.length === 0) {
      throw new Error(
        "OpenMeteo API returned an empty daily weather response array.",
      );
    }
    ...
    throw new Error(
      "An unexpected error occurred while retrieving daily weather data.",
    );
  }
}
