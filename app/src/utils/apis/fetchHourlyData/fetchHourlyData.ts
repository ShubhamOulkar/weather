 ...
 */
export async function fetchHourlyWeather({ latitude, longitude }: Cooradinates): Promise<HourlyWeather> {
    const url: string = import.meta.env.VITE_OPEN_METEO_HOURLY;

    if (!url) {
        throw new Error("Missing VITE_OPEN_METEO_HOURLY API endpoint environment variable.");
    }
    ...
    try {
        ...
        console.error("Unknown error during OpenMeteo hourly weather fetch:", error);
        throw new Error("An unexpected error occurred while retrieving hourly weather data.");
    }
}
