import type { CityData, OwmGeocodingResponse, Cooradinates } from "../../../types/types"
 ...
 */
export async function fetchLocationByCoords(coords: Cooradinates): Promise<CityData> {
    ...
    try {
        ...
        if (!res.ok) {
            throw new Error(`HTTP Error: Failed to retrieve location for coordinates. Status: ${res.status}`);
        }

        const data: OwmGeocodingResponse = await res.json(); 

        if (!data || data.length === 0) {
            throw new Error(`No recognizable location found for coordinates: (${lat}, ${lon}).`);
        }
        ...
        throw new Error(`An unexpected error occurred during reverse geocoding for (${lat}, ${lon}).`);
    }
}
